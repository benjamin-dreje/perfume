...
import Perfume from "../model/perfumeModel.js"; // ודא שהנתיב למודל שלך נכון

// טעינת קובץ ה-env. משתמשים ב-path כי קובץ ה-seed נמצא בתיקייה פנימית
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
dotenv.config({ path: path.resolve(__dirname, "../.env") });

// ==========================================
// 1. המידע הגולמי מהפרונטאנד
// ==========================================
const rawPerfumesFromFrontend = [
  // --- Men ---
  {
    id: 1,
    title: "Dior Sauvage",
...
// ==========================================
// 2. פונקציית המרה ל-JSON תקין
// ==========================================
function convertToJSON(data) {
  console.log("🔄 ממיר את המידע לפורמט JSON תקין...");
  return JSON.parse(JSON.stringify(data));
}

// ==========================================
// 3. פונקציית ההזרקה שקוראת מה-ENV
// ==========================================
async function startSeeding() {
  try {
    // משיכת מחרוזת החיבור מתוך ה-env של הפרויקט
    ...
    // המרה ל-JSON
    const cleanJsonData = convertToJSON(rawPerfumesFromFrontend);

    // מחיקת מוצרים ישנים (למניעת כפילויות)
    await Perfume.deleteMany({});
    console.log("cleared old data from perfume_shop collection.");

    // הזרקה דרך המודל שלך
    console.log("📤 adding new data...");
    await Perfume.insertMany(cleanJsonData);

    console.log(`✅ its in mongodb ${cleanJsonData.length}  -perfume_shop.`);
  } catch (error) {
